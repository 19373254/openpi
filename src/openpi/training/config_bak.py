...
_CONFIGS = [
    ...
    TrainConfig(
        # Change the name to reflect your model and dataset.
        name="pi0_libero-select_fruit_finetune100_yjj",
        # Here you define the model config -- In this example we use pi0 as the model
        # architecture and perform *full* finetuning. in the examples below we show how to modify
        # this to perform *low-memory* (LORA) finetuning and use pi0-FAST as an alternative architecture.
        model=pi0.Pi0Config(),
    ...
    TrainConfig(
        name="pi0_libero_low_mem_finetune-select_fruit100_yjj",
        # Here is an example of loading a pi0 model for LoRA fine-tuning.
        model=pi0.Pi0Config(paligemma_variant="gemma_2b_lora", action_expert_variant="gemma_300m_lora"),
        data=LeRobotVLABenchDataConfig(
            repo_id="select_fruit_none_distractor/select_fruit_none_distractor",
    ...
    TrainConfig(
        name="pi0_fast_libero-finetune-yjj",
        model=pi0_fast.Pi0FASTConfig(action_dim=7, action_horizon=10, max_token_len=180),
        data=LeRobotLiberoDataConfig(
            repo_id="YJJ_hf_test/libero",
            base_config=DataConfig(
                local_files_only=True,  # Set to True for local-only datasets.
                prompt_from_task=True,
            ),
    ...
    TrainConfig(
        name="pi0_fast-VLABench-finetune-yjj",
        model=pi0_fast.Pi0FASTConfig(action_dim=7, action_horizon=10, max_token_len=180),
        data=LeRobotVLABenchDataConfig(
            repo_id="VLABench_data_from_JuanJie/BaseM",
            base_config=DataConfig(
                local_files_only=True,  # Set to True for local-only datasets.
                prompt_from_task=True,
            ),
